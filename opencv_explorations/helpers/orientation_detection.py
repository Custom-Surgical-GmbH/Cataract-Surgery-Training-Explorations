...
#         red_polar, blue_polar, green_polar, black_polar))
    ...
    colors_stacked_aug[:, window_width //
                       2:colors_stacked_aug.shape[1]-window_width//2] = colors_stacked
    colors_stacked_aug[:, :window_width//2] = colors_stacked[:,
                                                             colors_stacked.shape[1]-window_width//2:]
    colors_stacked_aug[:, colors_stacked_aug.shape[1] -
                       window_width//2:] = colors_stacked[:, :window_width//2]
    entropy = []
    for i in range(360):
...
#     print(optimal_deg, optimal_degs)
    ...
    if return_verbose:
        ...
        cv2.putText(vis, 'red', (0, round(
            vis.shape[0]*0.00 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        cv2.putText(vis, 'blue', (0, round(
            vis.shape[0]*0.33 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        cv2.putText(vis, 'green', (0, round(
            vis.shape[0]*0.66 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
...
#         red_polar, blue_polar, green_polar, black_polar))
    ...
    if return_verbose:
        ...
        cv2.putText(vis, 'green', (0, round(
            vis.shape[0]*0.00 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        cv2.putText(vis, 'black', (0, round(
            vis.shape[0]*0.25 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        cv2.putText(vis, 'red', (0, round(
            vis.shape[0]*0.50 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
        cv2.putText(vis, 'blue', (0, round(
            vis.shape[0]*0.75 + 10)), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
