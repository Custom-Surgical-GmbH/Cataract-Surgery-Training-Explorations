...
def evaluate_limbus(methods, data_dir, append_frame_n=False):
    ...
    for filename in sorted(os.listdir(data_dir)):
        ...
        with open(os.path.join(data_dir, filename)) as f:
            data = json.load(f)
            limbus_shape = [shape for shape in data['shapes'] if shape['label'] == 'limbus']
            assert (len(limbus_shape) == 1), f'no unique \'limbus\' label in the file \'{filename}\''
    ...
    assert (len(image_filenames) == len(limbus_groundtruths)), 'different number of image_filenames vs. limbus_groundtruths'
    ...
    for i in range(len(image_filenames)):
        ...
        for method in methods:
            ...
            if append_frame_n:
                method_result = np.append(method_result, limbus_groundtruths[i][-1])
